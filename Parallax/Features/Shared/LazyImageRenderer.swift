...
/// repeated actor hops on every outer parent redraw. Shared by `JellyfinImage` (item
/// art) and `AccountAvatar` (user profile photo) — both render Jellyfin images that need
/// the session-scoped pipeline, so this is the one place that resolves it.
