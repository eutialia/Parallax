...
///
/// Deliberately a local copy of `ParallaxCoreTestSupport.FakeKeychain`: linking a
/// SECOND product of the ParallaxCore package (like `ParallaxCoreTestSupport`) into
/// this app-hosted test bundle drags in a duplicate ParallaxCore target next to the
/// host app's copy, and cross-boundary casts then fail at runtime (`catch let e as
/// AppError` stops matching errors thrown by app code). Package test targets have no
/// host app, so they use the shared fake.
